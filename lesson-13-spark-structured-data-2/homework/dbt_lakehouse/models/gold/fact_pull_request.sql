...
select
    ...
    cast(date_format(merged_at, 'yyyyMMdd') as int)          as merged_date_id,  -- NULL, якщо не змерджено
    state,
    is_merged,
    is_draft,
    additions,
    ...
    review_comments,
    hours_open,
    case when label_names is null then 0 else size(label_names) end as label_count  -- size(NULL) = -1
